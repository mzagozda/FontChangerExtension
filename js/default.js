!(function (a, b) {
  ...
  ($.extend({
    ...
    hasData: function (a) {
      return (
        (a = a.nodeType ? $.cache[a[$.expando]] : a[$.expando]),
        !!a && !e(a)
      );
    },
  ...
  ((yb = Hb.exec(xb.toLowerCase()) || []),
    ...
    $.extend({
      ...
      ajaxSetup: function (a, b) {
        return (
          b ? A(a, $.ajaxSettings) : ((b = a), (a = $.ajaxSettings)),
          A(a, b),
          a
        );
      },
...
(!(function (a) {
  ...
  $(document).ready(function () {
    ...
    var a = $("#font_family"),
      ...
      k = {};
    jQuery.extend(k, j), (k.type = "global");
    ...
    var ab = function (a) {
      ...
      normalizeStyleState = function (a, b) {
        var c = a || {},
          d = {
            type: b || c.type || "custom",
            font_family: {
              name: null,
              type: null,
            },
        ...
        if (c.font_family) {
          d.font_family.name = c.font_family.name || null;
          d.font_family.type = c.font_family.type || null;
          if (c.font_family.url) {
            d.font_family.url = c.font_family.url;
          }
        }

        d.font_style = c.font_style || null;
        d.font_weight = c.font_weight || null;
        d.font_size = c.font_size || null;
        return d;
      },
      refreshScopeTitle = function () {
        if (forceGlobalEdit) {
          $(".domain").text("Font Settings for Global");
          return;
        }
        if (currentDomain) {
          $(".domain").html("Font Settings for<br />" + currentDomain + ":");
          return;
        }
        $(".domain").text("Font Settings");
      },
      ad = function (a) {
        var b = null;
        if ("global" === a) {
      ...
      getSelectedFontPayload = function () {
        var b = a.val() || "",
          c = a.find("option:selected").data("type") || null;

        if (!b || "__default__" === b || "default" === c) {
          return {
            name: null,
            type: null,
          };
      ...
      hasStyleValues = function (a) {
        return !!(
          a &&
          ((a.font_family && a.font_family.name) ||
            a.font_style ||
            a.font_weight ||
            a.font_size)
        );
      },
      syncMasterToggle = function () {
        if (!bb.length) return;
        var a = getEditingState();
        masterEnabled = hasStyleValues(a);
        masterEnabled
          ? bb.attr("checked", "checked")
      ...
      setMasterState = function () {
        if (!masterEnabled) {
          $(".well").addClass("settings-disabled");
        ...
        $(".well").removeClass("settings-disabled");
        b.removeAttr("disabled");
        h.removeAttr("disabled");
        n(getEditingState());
      },
      getEditingState = function () {
        return forceGlobalEdit ? k : j;
      },
            ...
            if (i && i.url) {
              ...
              if ("custom" === j.type) {
                if (b && b.length && $("#font_family_chk").attr("checked")) {
                  j.font_family = getSelectedFontPayload();
                } else {
                  j.font_family = {
                    name: null,
                    type: null,
        ...
        m = function () {
          chrome.storage.local.get(["styles", "profiles"], function (a) {
            if (i && i.url) {
              ...
              if (a && a.styles) {
                if (a.styles.domain_styles && b && a.styles.domain_styles[b]) {
                  j = normalizeStyleState(a.styles.domain_styles[b], "custom");
                }
                if (a.styles.global_style) {
                  k = normalizeStyleState(a.styles.global_style, "global");
                }
              }
              if (d) {
                j = normalizeStyleState(d, "custom");
              } else if (!(a && a.styles && a.styles.domain_styles && b && a.styles.domain_styles[b])) {
                j = normalizeStyleState(null, "custom");
              }
              syncMasterToggle();
              aa = !0;
              applyEditorScope();
    ...
    var y = {},
      ...
      fontPreviewKnownGoogleFamilies = {},
      fontPreviewLoadedFamilies = {},
      fontPreviewQueue = [],
      fontPreviewTimer = null,
      fontPreviewAutoLoadTimer = null,
      fontPreviewAutoLoadCursor = 0,
      fontPreviewProgressIndex = 0,
      fontPreviewProgressLoaded = !1,
      fontPreviewProgressSaveTimer = null,
      fontPreviewPendingTimers = {},
      fontPreviewPendingSince = {},
      FONT_PREVIEW_VISIBLE_DELAY = 1000,
      MAX_PENDING_PREVIEW_TIMERS = 40,
      PREVIEW_DEBUG = !1,
      previewScrollTick = null,
      previewLog = function () {
        if (!PREVIEW_DEBUG || !console || !console.log) {
          return;
        }
        var a = Array.prototype.slice.call(arguments);
        a.unshift("[PopupPreview]");
        console.log.apply(console, a);
      },
      normalizePreviewName = function (a) {
        return (a || "").trim().toLowerCase().replace(/\s+/g, " ");
      },
      canonicalPreviewFamily = function (a) {
        var b = normalizePreviewName(a);
        if (!b) return null;
        return fontPreviewCanonicalFamilies[b] || null;
      },
      getActiveResultsContainer = function () {
        return (
          document.querySelector(".select2-drop-active .select2-results") ||
          document.querySelector(".select2-drop .select2-results")
        );
      },
      getVisibleGoogleFonts = function () {
        var a = getActiveResultsContainer();
        if (!a) return [];
        var b = a.getBoundingClientRect(),
          c = {},
          d = [];
        var e = a.querySelectorAll("li");
        for (var f = 0; f < e.length; f++) {
          var g = e[f],
            h = g.getBoundingClientRect();
          if (h.bottom <= b.top || h.top >= b.bottom) {
            continue;
          }
          var i = (g.textContent || "").trim();
          if (!i) {
            continue;
          }
          var j = normalizePreviewName(i);
          if (!fontPreviewKnownGoogleFamilies[j] || c[j]) {
            continue;
          }
          c[j] = !0;
          d.push(fontPreviewCanonicalFamilies[j] || i);
        }
        previewLog("visible", d.length);
        return d;
      },
      isPreviewFontVisible = function (a) {
        var b = getVisibleGoogleFonts();
        for (var c = 0; c < b.length; c++) {
          if (b[c] === a) return !0;
        }
        return !1;
      },
      trimPendingPreviewTimers = function () {
        var a = Object.keys(fontPreviewPendingTimers);
        if (a.length <= MAX_PENDING_PREVIEW_TIMERS) return;
        a.sort(function (a, b) {
          return (
            (fontPreviewPendingSince[a] || 0) -
            (fontPreviewPendingSince[b] || 0)
          );
        });
        var b = a.length - MAX_PENDING_PREVIEW_TIMERS;
        for (var c = 0; c < b; c++) {
          var d = a[c];
          clearTimeout(fontPreviewPendingTimers[d]);
          delete fontPreviewPendingTimers[d];
          delete fontPreviewPendingSince[d];
        }
      },
      flushGooglePreviewQueue = function () {
        ...
        previewLog("flush", a.length, "remaining", fontPreviewQueue.length);
      ...
      enqueueGooglePreviewFont = function (a, b) {
        var c = canonicalPreviewFamily(a);
        if (!c || fontPreviewLoadedFamilies[c]) {
          return;
        }
        previewLog("enqueue", c, b ? "priority" : "normal");
        fontPreviewLoadedFamilies[c] = !0;
        if (fontPreviewQueue.indexOf(c) === -1) {
          if (b) {
            fontPreviewQueue.unshift(c);
          } else {
            fontPreviewQueue.push(c);
          }
          fontPreviewQueue.sort(function (a, b) {
            var c = Object.prototype.hasOwnProperty.call(fontPreviewOrder, a)
                ? fontPreviewOrder[a]
                : Number.MAX_SAFE_INTEGER,
              d = Object.prototype.hasOwnProperty.call(fontPreviewOrder, b)
                ? fontPreviewOrder[b]
                : Number.MAX_SAFE_INTEGER;
            if (c === d) {
      ...
      scheduleGooglePreviewFont = function (a) {
        var b = canonicalPreviewFamily(a);
        if (!b || fontPreviewLoadedFamilies[b] || fontPreviewPendingTimers[b]) {
          return;
        }
        previewLog("schedule", b);
        fontPreviewPendingSince[b] = Date.now();
        fontPreviewPendingTimers[b] = setTimeout(function () {
          delete fontPreviewPendingTimers[b];
          if (isPreviewFontVisible(b)) {
            enqueueGooglePreviewFont(b);
          } else {
            enqueueGooglePreviewFont(b);
          }
          delete fontPreviewPendingSince[b];
        }, FONT_PREVIEW_VISIBLE_DELAY);
        trimPendingPreviewTimers();
      },
      scheduleVisiblePreviewFonts = function () {
        var a = getVisibleGoogleFonts();
        for (var b = 0; b < a.length; b++) {
          scheduleGooglePreviewFont(a[b]);
        }
      },
      resolveHoverGoogleFamily = function (b) {
        var c = normalizePreviewName(b);
        if (!c) {
          return null;
        }
        if (fontPreviewKnownGoogleFamilies[c]) {
          return fontPreviewCanonicalFamilies[c] || b;
        }
        if (a && a[0] && a[0].options) {
          for (var d = 0; d < a[0].options.length; d++) {
            var e = a[0].options[d];
            if (!e) continue;
            if (e.getAttribute("data-type") !== "google") continue;
            var f = normalizePreviewName(e.textContent || e.value || "");
            if (f === c) {
              return (e.textContent || e.value || "").trim();
            }
          }
        }
        return null;
      },
      bindPreviewHoverWatcher = function () {
        if (!window.jQuery) {
          return;
        }
        window
          .jQuery(document)
          .off(
            "mouseenter.fontPreviewHover",
            ".select2-drop-active li, .select2-drop li",
          )
          .on(
            "mouseenter.fontPreviewHover",
            ".select2-drop-active li, .select2-drop li",
            function () {
              var a = window.jQuery(this),
                b = a.find(".select2-result-label").first(),
                c = (b.length ? b.text() : a.text() || "").trim(),
                d = resolveHoverGoogleFamily(c);
              if (!d) {
                return;
              }
              enqueueGooglePreviewFont(d, !0);
            },
          );
      },
      ...
      al = function (b) {
        ...
        (b || []).forEach(function (a) {
          if (a && a.name) {
            var b = normalizePreviewName(a.name);
            fontPreviewKnownGoogleFamilies[b] = !0;
            fontPreviewCanonicalFamilies[b] = a.name;
            fontPreviewGoogleList.push(a.name);
          }
        });
        if (a.data("select2")) {
          a.select2("destroy");
        ...
        var defaultOption = $("<option>Default (No Change)</option>");
        defaultOption.val("__default__");
        defaultOption.data("type", "default");
        defaultOption.attr("data-type", "default");
        a.append(defaultOption);
        c.forEach(function (b, d) {
          var c = $("<option>" + b.name + "</option>");
          (c.data("type", b.type),
            c.attr("data-type", b.type),
            a.append(c),
            "google" === b.type && (fontPreviewOrder[b.name] = d));
        });
        a.select2({
        ...
        a.off("select2-open.fontPreviewLazy").on(
          "select2-open.fontPreviewLazy",
          function () {
    ...
    (chrome.storage.local.get(
      ["custom_fonts", "google_fonts_api_key"],
      function (b) {
      ...
      e.select2({
        ...
        formatResult: buildWeightMarkup,
        formatSelection: buildWeightMarkup,
      }),
      c.select2({
        minimumResultsForSearch: -1,
        escapeMarkup: function (a) {
        ...
        formatResult: buildStyleMarkup,
        formatSelection: buildStyleMarkup,
      }),
      ...
      $("#save-profile-popup").click(function () {
        var c = forceGlobalEdit ? k : j;
        if (masterEnabled && $("#font_family_chk").attr("checked")) {
          c.font_family = getSelectedFontPayload();
        } else {
          c.font_family = {
            name: null,
            type: null,
        ...
        chrome.storage.local.get(["styles", "profiles"], function (a) {
          ...
          if (forceGlobalEdit) {
            a.styles.global_style = masterEnabled
              ? normalizeStyleState(c, "global")
              : {};
            chrome.storage.local.set({ styles: a.styles }, function () {
              if (chrome.runtime.lastError) {
                console.error("Unable to save global settings. Please try again.");
                return;
              }
              console.info("Global settings saved");
            });
            return;
          }
          ...
          if (!masterEnabled) {
            delete a.styles.domain_styles[d];
            delete a.profiles[d];
            chrome.storage.local.set(a, function () {
              ...
              console.info("Profile disabled for " + d);
            });
            return;
          }

          a.styles.domain_styles[d] = c;
          a.profiles[d] = {
            domain: d,
            style: {
              ...
              font_style: c.font_style || null,
              font_weight: c.font_weight || null,
              font_size: c.font_size || null,
            },
          };
          chrome.storage.local.set(a, function () {
            if (chrome.runtime.lastError) {
            ...
            console.info("Profile saved for " + d);
          });
        });
      }),
      ba.change(function () {
      ...
      bb.change(function () {
        masterEnabled = !!$(this).attr("checked");
        setMasterState();
        if (masterEnabled) {
          ag(getEditingState());
          return;
      ...
      $(".enable-select").change(function () {
        ...
        ((b = getEditingState()),
          $(this).attr("checked")
            ? ((b.font_family = getSelectedFontPayload()), a.select2("enable"))
            : ((b.font_family = {
                name: null,
                type: null,
              }),
      ...
      c.change(function () {
        ensureMasterEnabled();
        var a = null;
        ((a = getEditingState()), (a.font_style = $(this).val()), ag(a));
      }),
      e.change(function () {
        ensureMasterEnabled();
        var a = null;
        ((a = getEditingState()), (a.font_weight = $(this).val()), ag(a));
      }),
      g.keyup(function () {
        ensureMasterEnabled();
        var a = null;
      ...
      a.change(function () {
        ...
        var b = null;
        b = getEditingState();
        var c = getSelectedFontPayload();
        ((b.font_family = c),
          "google" === c.type && c.name && enqueueGooglePreviewFont(c.name, !0),
          ag(b));
      }),
      $(".done").click(function () {
        var a = masterEnabled ? getEditingState() : {};
        clearTimeout(ae);
        o(a);
        window.close();
      }));
